...
export function initCreateScreen(root: HTMLElement) {
  ...
  const hashInput = <input type="text" value={initialHash} class="form-input" placeholder="Bitcoin Block Hash" /> as HTMLInputElement;
  const contentInput = <textarea class="form-textarea" placeholder="Enter options, one per line. Duplicates increase weight."></textarea> as HTMLTextAreaElement;
      ...
      <div class="card">
        ...
        <div>
          <button type="button" onclick={() => {
            let hash: string | undefined = hashInput.value.trim();
            ...
            const options = Object.entries(weights).map(([label, weight], index) => ({
              label,
              weight,
              // Generate a color based on index/hash or random?
              // Let's use a simple palette or random HSL
